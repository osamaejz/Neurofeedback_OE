...
try:
    ...
    # interested in it)
        ...
        current_2k_samples.append(sample[nf_channel])
        
        if(len(current_2k_samples) == 2000):
                        
            check = False

            real_pwelch = signal.welch(current_2k_samples, fs=Fs, window='hanning', nperseg=wL, noverlap=wL/2, nfft=wL)
            
            real_beta = real_pwelch[1][int(wL*15/Fs) : int((wL*20/Fs)) + 1]
    ...
    while check_2:      
        ...
        current_2k_samples.pop(0) # delete value at first index 
        
        current_2k_samples.insert(1999, sample[nf_channel]) # insert new sample at last index
        ...
        if (len(complete_samples) >= (NF_time * 500)):#(t2-t1 >= NF_time):# total time for nf session
            ...
            other_data_df = pd.concat((NFB_frequency_df,Performance_df), axis =1)
